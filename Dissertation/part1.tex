\chapter{Математические модели открытых квантовых системы}\label{ch:ch1}
...
В разделе \cref{sec:ch1/qj} будет рассмотрен микроскопический подход к описанию и анализу открытых квантовых систем "--- метод квантовых траекторий.
...
\begin{equation}
	\label{eq:shr}
	\begin{gathered}
		\ket{\dot{\psi}(t)} = -\iix \opHx \ket{\psi(t)},
	\end{gathered}
\end{equation}
...
Квантовые измерения описываются набором $\left\{{M}_k\right\}_{k=1}^{N}$ операторов измерения, которые удовлетворяют условию $\sum_k {M}_k^\dagger{M}_k = {I}$. Заданное состояние $\ket{\psi}\in\mathcal{H}$ после измерения становится:
\begin{equation}
...
Предположим теперь, что квантовая система находится в состоянии $\ket{\psi_1}$ с вероятностью $q_1$, в состоянии $\ket{\psi_2}$ с вероятностью $q_2$ и так далее. То есть, имеется ансамбль состояний $\{ q_i, \ket{\psi_i} \}_{i=1}^{N}$, которые называются «чистыми». Квантовые измерения для заданного состояния системы $\ket{\psi_i}$ и операторов $\{ M_k\}$ осуществляются следующим образом:
\begin{equation}
	...
	\end{gathered}
\end{equation}
с соответствующими вероятностями ${p}_{k|i}=\braket{\psi_i| M_k^{\dagger} M_k | \psi_i}$ получения исхода $k$ при заданном состоянии $\ket{\psi_i}$.
...
\section{Метод квантовых траекторий}\label{sec:ch1/qj}
...
\begin{algorithm}
	...
	\For{\(j\leftarrow 1\) \KwTo \(M_r\)}
	{
		...
		\While{\(t_j < t^C\)}{
			Генерация случайной величины \(\eta\), равномерно распределённой в интервале \(\left[0; 1\right] \)\;
			...
			Трансформация волновой функции с учётом выбранного канала диссипации: \( \left| \psi_j (t_j) \right\rangle \leftarrow \frac{ V_k(t_j) \left| \psi_j (t_j) \right\rangle}{\norm{V_k(t_j) \left| \psi_j(t_j) \right\rangle}}\)\;
		}
	}
		...
		\For{\(s_d\leftarrow 1\) \KwTo \(2\)}{
			...
			\uIf{d = D}{
				\uIf{\( \norm{| \tilde{\psi}_j(t) \rangle}^2 < \eta\)}{
					...
					\(\eta \leftarrow \) новое случайно сгенерированное число\;
				}
...
\section{Выводы по главе}\label{sec:ch1/results}
...
%
%\subsection{Ненумерованные одиночные формулы}\label{subsec:ch2/anderson/sub1}
%
...
%
%\subsection{Ненумерованные многострочные формулы}\label{subsec:ch2/anderson/sub2}
%
...
%\]
%
%\subsection{Нумерованные формулы}\label{subsec:ch2/anderson/sub3}
%
