...
\begin{algorithm}[H]
	...
	\For{\(j\leftarrow 1\) \KwTo \(M_r\)}
	{
		...
		\While{\(t_j < t^F\)}{
			...
			Осуществление квантового скачка через диссипативный оператор \(V_k(t_j)\) с вероятностью \(p_k\leftarrow\gamma_k(t_j) \frac{\norm{ V_k(t_j) \left| \psi_j(t_j) \right\rangle}^2}{\sum_{l=1}^{K} \norm{ V_l(t_j) \left| \psi_j(t_j) \right\rangle}^2}\)\;
			Трансформация волновой фунции с учетом выбранного канала диссипации: \( \left| \psi_j (t_j) \right\rangle \leftarrow \frac{ V_k(t_j) \left| \psi_j (t_j) \right\rangle}{\norm{V_k(t_j) \left| \psi_j(t_j) \right\rangle}}\)\;
		}
	}
...
\begin{algorithm}[H]
	...
	Инициализация количества пройденных шагов на каждой глубине бисекции: \(s_d = 0\) \((d=1 \ldots D)\)\;
	\label{alg:qt_bisection}
	\caption{Прогагация квантовой траектории на время \(\tau\) при помощи метода бисекции}
\end{algorithm}
\DecMargin{1em}
...
\begin{algorithm}[H]
	...
	\Fn{\prop{\(\left| \psi_j (t) \right\rangle\), \(t\), \(\eta\), \(d\)}}{
		\uIf{d = D}{
			\While{$s_d \neq 2$}{
				Пропагация \( | \tilde{\psi}_j(t) \rangle \leftarrow \mathcal{P}_{\tau_d} \left| \psi_j(t) \right\rangle \)\;
				\uIf{\( \norm{| \tilde{\psi}_j(t) \rangle}^2 < \eta\)}{
					\jump{\(| \tilde{\psi}_j(t) \rangle\)}\;
					Случайная генерация числа \(\eta\)\;
				}
		...
		\Else{
			\While{$s_d \neq 2$}{
				...
				\Else{
					\(\left| \psi_j(t) \right\rangle = | \tilde{\psi}_j(t) \rangle\)\;
				}
				\(s_d \leftarrow s_d + 1\)\;
				\(t \leftarrow t + \tau_d\)\;
			}
